...
def log_train(batch_id, batch_num, metric, step_loss, log_interval, epoch_id, learning_rate, speed):
    ...
    logging.info(train_str, epoch_id + 1, batch_id + 1, batch_num, step_loss / log_interval,
                 learning_rate, *metric_val, speed)
...
def train(metric):
    ...
    opt = hvd.DistributedOptimizer(opt, comp_alg="topkadam", threshold=262143)
    trainer = gluon.Trainer(all_model_params, opt,
        ...
        if not only_inference:
            ...
            for batch_id, seqs in enumerate(train_data):
                ...
                if not accumulate or (batch_id + 1) % accumulate == 0:
                    ...
                    # trainer.update(accumulate if accumulate else 1, allreduce=True)
                ...
                if rank == 0 and (batch_id + 1) % (args.log_interval) == 0:
                    speed = hvd.size() * args.batch_size * args.log_interval / (time.time() - tic)
                    log_train(batch_id, len(train_data), metric, step_loss, args.log_interval,
                              epoch_id, trainer.learning_rate, speed)
                    tic = time.time()
                    step_loss = 0
